...
export class MediapipeHands extends Behaviour {
    ...
    async awake() {
        ...
        await this.startWebcam(this._video);

        // parenting video to shadowRoot for background video effect
        if (this.context.domElement.shadowRoot) {
            // set video styles directly, because css styles are not working in shadow root
